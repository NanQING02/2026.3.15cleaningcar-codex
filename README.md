...
- 主事件只有 `type=5` 会附加 `wheelResults`
- 当前车轮结果优先按单车生命周期锁定：主轨迹生命周期内一旦命中左右轮结果，会分别锁到该车；`type=5` 时优先附加这份锁定结果
- 当生命周期内尚未锁到结果时，才退回 `wheel.bind_window_seconds` 时间窗内的回查逻辑
...
- 车轮中心安全边距默认值：`wheel.center_min_margin_ratio=0.25`
